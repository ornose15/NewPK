...
if(loggedIn() && getLevel() >= 2) {
	function delCom() {
		...
		if((loggedIn() && getLevel() >= 2) || ($_SERVER['REMOTE_ADDR'] == $row['ip'])) {
			...
			<div class='content'><div style='padding:5px'>Comment was successfully deleted.<br />You are being redirected.</div></div>";
			echo "<meta http-equiv='REFRESH' content='2;url=admin.php?action=comments'>";
		}
		return 1;
	}
	delCom();
